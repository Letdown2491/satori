...
function prune(): void {
    ...
    let over = cache.size - CAP;
    for (const k of cache.keys()) { if (over-- <= 0) break; cache.delete(k); }
}

const relaySet = (r: RelayList | null): string[] =>
    (r ? [...new Set([...r.read, ...r.write, ...INDEXER_RELAYS])] : INDEXER_RELAYS).slice(0, 8);
...
export async function fetchReplyPresence(pool: Pool, myRelays: RelayList | null, noteIds: string[], follows: Set<string>, me: string): Promise<void> {
    ...
    const events = await pool.query(relaySet(myRelays), { kinds: [1], '#e': todo, limit: LIMIT }).catch(() => [] as NostrEvent[]);
...
export async function fetchArticleReplyPresence(pool: Pool, myRelays: RelayList | null, naddrs: string[], follows: Set<string>, me: string): Promise<void> {
    ...
    for (const n of todo) { try { const d = decode(n); if (d.type === 'naddr') byAddress.set(`${d.data.kind}:${d.data.pubkey}:${d.data.identifier}`, n); } catch { /* skip */ } }
